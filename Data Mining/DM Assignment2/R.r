data<-read.csv("twomillion.csv",header=FALSE)
setwd("C:\\Users\\aishwarya\\Desktop\\Data Mining\\DM Assignment2\\")
data<-read.csv("twomillion.csv",header=FALSE)
sam<-sample(seq(1,length(data[,1])), 10000, replace=T)
my_sample<-data[sam,1]
mean(my_sample)
mean(my_sample)
max(my_sample)
my_sample<-data[sam,1]
mean(my_sample)
max(my_sample)
var(my_sample)
var(my_sample)
quantile(my_sample,.25)
my_sample<-data[sam,1]
mean(my_sample)
max(my_sample)
var(my_sample)
quantile(my_sample,.25)
mean(data[,1])
max(data[,1])
var(data[,1])
quantile((data[,1]),.25)
abs(mean(my_sample)-mean(data[,1]))
abs(max(my_sample)-max(data[,1]))
abs(var(my_sample)-var(data[,1]))
abs(quantile(my_sample,.25)-quantile((data[,1]),.25))
 write.csv(my_sample,"my_sample.csv")
ca_data<-read.csv("CA_house_prices.csv",header=FALSE)
oh_data<-read.csv("OH_house_prices.csv",header=FALSE)
boxplot(ca_data[,1],oh_data[,1],col="blue",main="Aiswarya Chundury",names=c("CA houses", "Ohio houses"),ylab="Prices (in thousands)")
hist(ca_data[,1]*1000,breaks=seq(0,3500000,by=500000),col="red",xlab="Prices",ylab="Frequency",main="Aiswarya Chundury")
hist(ca_data[,1]*1000,breaks=seq(0,3500000,by=500000),col="blue",xlab="Prices",ylab="Frequency",main="Aiswarya Chundury")
plot(ecdf(ca_data[,1]),verticals= TRUE,do.p = FALSE,main ="Aiswarya Chundury",xlab="Prices (in thousands)",ylab="Frequency")
lines(ecdf(oh_data[,1]),verticals= TRUE,do.p = FALSE,col.h="blue",col.v="red",lwd=4)
legend(2100,.6,c("CA Houses","OH Houses"), col=c("brown","black"),lwd=c(1,4))
football<-read.csv("football.csv", header=TRUE)
plot(football[,2],football[,3],xlim=c(0,12),ylim=c(0,12),pch=15,col="red",xlab="2003 Wins",ylab="2004 Wins",main="Aiswarya Chundury")
abline(c(0,1))
cor(football[,2],football[,3])
cor(football[,2],football[,3])
cor(football[,2],football[,3]+10)
cor(football[,2],football[,3]*2)
cor(football[,2],football[,3]*-2)
median(oh_data[,1])
mean(oh_data[,1])
median(oh_data[,1]+10)
median(oh_data[,1]*2)
sd(ages+10)
ages<-c(19,23,30,30,45,25,24,20)
sd(ages)
sd(ages+10)
sd(ages*10)

